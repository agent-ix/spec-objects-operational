...
- Support models emitted as sibling files: the open marker schemas
  ...
  `EvidenceRef`; and the closed enums `ConfigurationScope`,
  `RollbackStrategy`, `RolloutStrategy`, `Comparator`, `AlertSeverity`, and
  `EvidenceKind`.
...
|---|---|---|---|---|
| configuration | `Configuration` | `fields` | `scopes: ScopeAssignment[]`, `configures: SemanticId[]`, `clauses`, `operations` | `fields` has ≥ 1 item and 0 identity fields: a parameter set is keyed by the configured object, not by itself; `relations` forbidden |
| migration | `Migration` | `fields`, `clauses` | `rollback: RollbackDecl`, `migrates: SemanticId[]`, `dependsOn: SemanticId[]`, `operations` | `fields` has ≥ 1 item and 0 identity fields; `clauses` has ≥ 1 item — a migration declares the safety condition it preserves; `relations` forbidden |
| sli | `Sli` | `fields` | `measures: SemanticId[]`, `clauses` | `fields` has ≥ 1 item and ≥ 1 measured field; `operations` forbidden — an SLI declares a measurement, not behaviour |
| slo | `Slo` | `fields`, `clauses` | `objective: ObjectiveDecl`, `constrains: SemanticId[]`, `dependsOn: SemanticId[]` | `fields` has ≥ 1 item and ≥ 1 measured field; `clauses` has ≥ 1 item — the objective is an invariant over the SLI; `operations` forbidden |
...
| deployment | `Deployment` | `fields`, `operations` | `rollout: RolloutDecl`, `deploys: SemanticId[]`, `dependsOn: SemanticId[]`, `relations`, `clauses` | `fields` has ≥ 1 item and ≥ 1 identity field (the release identity); `operations` has ≥ 1 item (the lifecycle actions) |
...
- `EvidenceRef` SHALL be `{ kind: EvidenceKind, record: SemanticId, doc?: string }` with `EvidenceKind` the closed set `exercise`, `execution`, `observation`, `review`.
...
- An `EvidenceRef.record` SHALL be a `SemanticId` naming a record of the `agent-ix/quoin#267` operational evidence family.
...
- Every cross-reference a declaration makes (`type.target`, `configures`, `measures`, `constrains`, `dependsOn`, `migrates`, `deploys`, `escalatesTo`, `references`, `correlates`, `breaches`, `triggers`, `EvidenceRef.record`, `ObjectiveDecl.sli`, `AlertCondition.sli`) SHALL be a `SemanticId` or `KernelScalar` per semantic-core, so a bare token is rejected by the schema; resolution against the bundle, and the placeholder `ix://<org>/<repo>/unresolved/<Token>` with its `semantic.unresolved-type` finding, exist today for `type.target` only (quire-rs FR-070) and for the other keys once `agent-ix/quoin#335` publishes their mapping.
...
| FR-004-AC-10 | The empty record `{}` fails all eight object-type schemas, and a record carrying an `observations` key fails every one of them. | Test |
| FR-004-AC-11 | `evidence` is declared by `Incident.json` and by no other shipped schema; an `evidence` entry whose `record` is a bare token rather than a `SemanticId` fails, and one whose `kind` is outside `EvidenceKind` fails. | Test |
