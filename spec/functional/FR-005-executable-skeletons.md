...
- A bundle index built over these skeletons SHALL carry one entry per frontmatter `id`, not one per file, because the resolver matches entries rather than distinct ids and would otherwise raise `semantic.ambiguous-type` on a `Type` cell naming a title two files share.
- The test suite SHALL verify FR-005-AC-3 through `extract_semantic` with that index and not through `validate_document`, which builds an index carrying `imports` only, so every non-kernel token falls to the `no-bundle-index` placeholder there; supplying an index to `validate_document` is `agent-ix/quire-rs`' concern, not this module's.
...
- Three further overlaps run the other way — the kernel form is required and the typed key is optional and unpopulated — and for each of them the kernel form SHALL remain the authority until `agent-ix/quoin#335` publishes the mapping that fills the typed key: the alert `## Flow` mermaid fence carries the burn-rate thresholds, windows and severities that `conditions` will carry; the slo frontmatter `target`/`window` pair carries the promise that `objective` will carry; the incident `## Timeline` carries the ordering that `evidence` and the occurrence fields will carry.
- Where the kernel form is the authority, no test SHALL assert agreement between it and its typed key, because the typed key is absent from every extracted record today; agreement becomes assertable only when the mapping lands.
- Each negative fixture SHALL fail `validate_document` with an error whose message carries the fixture's `expect:` code, covering at least: a configuration with an identity row (`semantic.record-invalid`), a migration with no `## Invariants` (`semantic.record-invalid`), a migration carrying an `applied_at` `Timestamp` row (`semantic.record-invalid`), an SLI whose only field carries no unit (`semantic.record-invalid`), an alert carrying a `## Properties` table (`semantic.record-invalid`), a runbook whose `## Operations` declares no operation (`semantic.record-invalid`), an incident with no `Timestamp` row (`semantic.record-invalid`), a `## Properties` section carrying both a table and a fence (`semantic.properties-both-forms`), an operation whose `Post:` names an undeclared clause (`semantic.dangling-clause-ref`), and a `Type` token that is not an `Identifier` (`semantic.invalid-type-token`); the last three re-check the engine's published diagnostics under this module's schemas rather than re-specify them.
- Because seven of the ten fixtures expect the same `semantic.record-invalid` code, each such fixture SHALL additionally carry a `detail:` frontmatter key naming a substring of the failing schema location, keyword, or offending declaration that no other fixture's message carries.
...
| FR-005-AC-5 | Every negative fixture fails validation with an error message containing both its `expect:` code and its `detail:` substring, and at least the ten cases listed in Behavior are present. | Test |
...
| FR-005-AC-10 | With the Quire wheel absent from the environment, every semantic test fails — none is skipped or reported green — with a message naming `extract_semantic` and `poetry install`. | Test |
| FR-005-AC-11 | `pyproject.toml` declares `quire` as a dev dependency pinned to the `internal-pypi` source. | Test |
