...
this test, a module that Quire accepts but Quoin refuses would ship. This test
verifies [FR-003](../functional/FR-003-semantic-manifest-contract.md).
...
The module directory `spec_objects_operational/` from this branch, containing
...
1. Record `quoin module` output before the install.
   - IT-002-SC-01: the listing is captured, including any existing
     `spec-objects-operational` entry.
2. Run `quoin module install path:<checkout>/spec_objects_operational`.
   - IT-002-SC-02: exit code 0 and no `semantic.*` diagnostic at `error` severity.
3. Run `quoin module`.
   - IT-002-SC-03: the listing contains `spec-objects-operational` sourced from
     the path.
4. Inspect the installed module root.
   - IT-002-SC-04: `semantic/package-manifest.json` exists with
     `package.identity` `agent-ix/spec-objects-operational` and one export per
...
package manifest names the eight exports, and the prior module state is
